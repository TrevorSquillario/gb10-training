...
if [ -d "~/gb10/scratch/${PDB_ID}" ]; then
    echo "--- ~/gb10/scratch/${PDB_ID} exists; removing to ensure clean workspace ---"
    rm -rf "~/gb10/scratch/${PDB_ID}"
fi
mkdir -p "~/gb10/scratch/${PDB_ID}"
cd "~/gb10/scratch/${PDB_ID}" || { echo "Failed to enter ~/gb10/scratch/${PDB_ID}"; exit 1; }
...
echo "--- Files prepared in ~/gb10/scratch/${PDB_ID} ---"
echo "------------------------------------------------------"
